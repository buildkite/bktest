...
* Remove the experimental `otel_instrumentations` option (passing it now raises
  ...
  instrumentation by requiring the gems you want; the collector installs every
  registered instrumentation when it configures the SDK, and installs none when
  the suite configures the SDK itself.
