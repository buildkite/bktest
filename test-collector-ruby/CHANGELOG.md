...
* Bound experimental RSpec OpenTelemetry failure detail: 10,240 bytes per
  message, 16,384 per stacktrace, and 1,024 for status descriptions, with a
  shared 26 KiB exception-event budget and a 100-event cap including omission
  summaries. Truncation preserves valid UTF-8 and includes a visible marker.
* Cap collector-set OpenTelemetry test-span string attributes at 1,024 bytes,
  replacing invalid UTF-8. Annotations and resource attributes remain uncapped.
* Limit test-span export batches to 240 spans. Add positive-integer
  ...
  8192). Invalid values warn and fall back; batches above 240 warn and clamp.
